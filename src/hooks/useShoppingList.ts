...
export const useShoppingList = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const itemsSub = db.shoppingList.find().$.subscribe((foundItems: ShoppingListItem[]) => {
  ...
  // Add item
  const addItem = async (itemData: Omit<ShoppingListItem, 'id' | 'createdAt'>) => {
    try {
      const db = await getDatabase();
      const now = Date.now();
      ...
      await db.shoppingList.insert(newItem);
  ...
  const toggleItemCheck = async (id: string, isChecked: boolean) => {
    try {
      const db = await getDatabase();
      const item = await db.shoppingList.findOne(id).exec();
  ...
  const clearCheckedItems = async () => {
    try {
      const db = await getDatabase();
      const checkedItems = await db.shoppingList.find({
        selector: {
          isChecked: true
        }
      }).exec();
  ...
  const addFromPantry = async (filter: 'finished' | 'expired' | 'all' = 'all') => {
    try {
      const db = await getDatabase();
      ...
      const pantryItems = await db.items.find({
        selector
      }).exec();
      ...
      await db.shoppingList.bulkInsert(shoppingItems);
