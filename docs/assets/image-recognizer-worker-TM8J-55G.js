...
`)}function Ts(e,i){return e.endsWith(i)?e.slice(0,-i.length):e}function Z(e,i){if(!e)throw new Error(i)}const Cs=()=>({type:"WORKER_READY",output:{initialized:!0}}),Us=e=>({status:"complete",output:e,type:"CATEGORIZE_IMAGE"}),rt=e=>({status:"error",output:e,type:"CATEGORIZE_IMAGE"}),Os=e=>({status:"complete",output:{isValid:e},type:"TOKEN_VALIDATION"}),Ms=e=>({status:"error",output:e,type:"TOKEN_VALIDATION"}),B=console.log.bind(console,"[ImageRecognizerWorker]"),xe=console.error.bind(console,"[ImageRecognizerWorker]"),K=e=>{B("Sending message to main thread:",e),self.postMessage(e)};let Q=null;const st=async e=>{B("Initializing client with token:",e?"***":"empty");try{return Z(e,"Token is required"),Z(e.startsWith("hf_"),"Invalid token format"),Q=new fs(e),Z(Q,"Client initialization failed"),await Q.textGeneration({model:"gpt2",inputs:"Hello",parameters:{max_new_tokens:1,do_sample:!1}}),B("Client initialized and validated successfully"),!0}catch(i){return Q=null,B("Client initialization error:",i),!1}};self.addEventListener("message",async e=>{B("Received message from main thread:",e.data);try{switch(e.data.type){case"VALIDATE_TOKEN":{B("Validating token...");const{token:i}=e.data;try{const t=await st(i);B("Token validation result:",t),K(Os(t))}catch(t){const a=t instanceof Error?t.message:String(t);xe("Token validation error:",t),K(Ms(a))}break}case"CATEGORIZE_IMAGE":{B("Categorizing image...");const{imageData:i}=e.data,t=e.data.token||"";if(!Q&&!await st(t)){K(rt("Failed to initialize client: Invalid token"));return}try{Z(i,"Image data is required"),Z(i instanceof Blob,"Image data must be a Blob"),Z(Q,"Client is not initialized");const a=await Q.zeroShotImageClassification({inputs:i,model:"openai/clip-vit-large-patch14",parameters:{candidate_labels:["fruits-vegetables","dairy-and-eggs","meat-fish","grains","canned-goods","spices","snacks","beverages","other"]},provider:"hf-inference"});B("Classification result:",a),K(Us(a))}catch(a){const n=a instanceof Error?a.message:String(a);xe("Classification error:",a),K(rt(n))}break}default:throw new Error(`Unknown message type: ${e.data.type}`)}}catch(i){const t=i instanceof Error?i.message:String(i);xe("Handler error:",i),K({status:"error",output:t,type:e.data.type})}}),K(Cs())})();
